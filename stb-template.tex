...
In the very rare cases where the conditions that bind the positions of the mobile points form what we have called \textit{obligatory movements}, and thus the equations to which the coordinates of points must satisfy are such that time enters into these equations; the virtual velocities are then the infinitesimally small spaces that the different points would describe simultaneously, if they were made to leave their position without changing the state of connection, as it was at a determined moment; that is to say, assuming that in contradiction with the conditions of the problem, the obligatory movement would be stopped from that moment, and that, so to speak, time has been suspended in the equations that expressed these movements.
